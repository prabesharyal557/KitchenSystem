...
Open http://127.0.0.1:3000 on the server computer and create the first manager account. There are no shared demo PINs. Initial manager setup is allowed only from localhost. Add waiter and kitchen accounts in **Staff & payroll**. Run integration checks with `npm test`.
...
- Redesigned responsive manager, waiter and kitchen screens.
...
- The kitchen board has two queues: **New orders** and **Ready to serve**. Kitchen staff mark food ready in one step; waiters mark it served from their Ready to serve page. Legacy preparing tickets appear under New orders. Managers can also complete either step and collect payment from the served-table list.
- Kitchen staff receive new-order alerts; waiters receive ready-to-serve alerts, with table numbers and items. In-app alerts work automatically, including while typing. Use **Enable sound & desktop alerts** in the staff workspace for sound and browser notifications. Desktop notifications require browser permission and browser support; keep the app open. Alerts are deduplicated per staff member in each browser tab, including reloads, and disappear when the order moves on.
...
- Manager-controlled closure revokes waiter/kitchen sessions. Reopening allows active accounts to sign in again.
